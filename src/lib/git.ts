...
export function getGitDiff(projectPath: string, filePath: string, status: string): Promise<string> {
  return invoke<string>("get_git_diff", { projectPath, filePath, status });
}
